<template>
  <confirm-permanent
    ...
    @delete-note="deletePermanently"
  />
  <div v-if="this.$store.state.theFilteredDeleted.id">
    <div class="deletedHeader">
      <div @click="toConfirmDelete" class="delete_permanently">
        <p>
          <span><i class="fi fi-rr-trash-xmark"></i></span> Delete permanently
        </p>
      </div>
      <div class="restore">
        <p>
          <span><i class="fi fi-rr-time-past"></i></span> Restore note
        </p>
      </div>
    </div>
    <div class="header">
      <input
        class="title"
        :value="$store.state.theFilteredDeleted.title"
        ...
        <div class="clm">
          ...
          <div>
            <p class="created-date">
              {{ $store.state.theFilteredDeleted.date }}
            </p>
          </div>
        </div>
        <div class="clm">
    ...
    <div class="content">
      <div
        ...
        v-if="
          !this.$store.state.theFilteredDeleted.content ||
          this.$store.state.theFilteredDeleted.content.length === 0
        "
      >
        <img src="../Video/system-regular-18-autorenew.gif" alt="Animated GIF" class="restore_animation"/>
        <h1>RESTORE NOTE TO MAKE CHANGES</h1>
      </div>
      ...
      <textarea
        ...
        disabled
        class="the-content"
        :value="this.$store.state.theFilteredDeleted.content"
...
import ConfirmPermanent from "./ConfirmPermanent.vue";

export default {
  data() {
    return {
      content: this.$store.state.theFilteredDeleted.content,
      confirmDelete: false,
